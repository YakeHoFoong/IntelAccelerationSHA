...
static inline void __cpuidex(int32_t cpuInfo[4], int32_t function_id, int32_t subfunction_id)
{
    cpuInfo[0] = function_id;
    cpuInfo[2] = subfunction_id;
    cpuid(&cpuInfo[0], &cpuInfo[1], &cpuInfo[2], &cpuInfo[3]);
    /*
...
static inline void __cpuid(int32_t cpuInfo[4], int32_t function_id)
{
    ...
    _asm {
        ...
        mov edi, cpuInfo; // Prepare to address CPUInfo with edi
        mov eax, function_id;
...
#if defined(_MSC_VER ) && defined(_WIN64)
extern "C" {
    __declspec(dllexport) void print_cpu_extensions(void);
}
#endif
...
// Print out supported instruction set extensions for testing
